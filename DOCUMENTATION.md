...
This is an asynchronous document-parsing service. A user drops a single file — an image, a scanned PDF, a text or Word document — onto a web page, and the file goes straight from their browser to an object store without ever passing through this application's server memory. A background worker then picks the file up, extracts its readable content, sends it to Google Gemini 2.5 Flash (the free-tier multimodal API), and turns the model's raw response into a strict, Zod-validated JSON object describing the document: what type it is, a verbatim transcription, the merchant, date, total, tax, currency, category, and any itemised line items. The user watches the job progress through `PENDING` → `PROCESSING` → `DONE` on a polling page, then reviews the structured result and can ask for a single follow-up executive summary, generated by DeepSeek (paid, OpenAI-compatible) and cached permanently in the database.
...
**Why it is needed.** Every parse is a Gemini API call, the free tier rate-limits requests per minute, and DeepSeek bills for every summarization. With unlimited parallelism, ten users uploading at once would send ten simultaneous model calls, blow through the rate/budget allowance, and get HTTP 429 errors — which is not just a failed parse, it is a *paid- or quota-costing* failed parse plus back-off chaos across every job. A cap of three means the service can never present the model APIs with more concurrent demand than the plan tolerates, exchanging throughput for predictability.
...
**Why it is needed.** Scattered literals rot: an engineer tuning the summarizer bumps `temperature` in the route, misses the worker copy, and the two never agree again; a model upgrade becomes a hunt through five files where one instance is inevitably missed; and a reviewer cannot tell at a glance what model or budget the system actually runs on. This product is an assessment of exactly that discipline, so the config file is the contract: one place to read the parsing model `gemini-2.5-flash`, the summary model `deepseek-chat`, the temperature, token caps, retries, and the concurrency cap.

**How I implemented it.** Everything lives in `src/config/ai.config.ts`. The worker never names a model — it calls `ai.models.generateContent({ model: GEMINI_CONFIG.parsing.model, ... })` in `src/inngest/functions/parseReceipt.ts`, and the summarize route builds its DeepSeek client from `DEEPSEEK_CONFIG` (`model`, temperature, token cap, base URL) in `src/app/api/jobs/[id]/summarize/route.ts`. The system prompts are exported alongside the config from the same module so prompt and parameters can never drift apart.
...
### 5.5 Multimodal Vision Input (Google Gemini Vision)
...
The model is identified by config (`GEMINI_CONFIG.parsing.model`), and the response's `candidate.finishReason` is checked to be `"STOP"` — truncated (`MAX_TOKENS`) or blocked (`SAFETY`) outputs throw immediately instead of being half-parsed.

**What I chose against, and why.** I rejected sending the scan to DeepSeek — it has no vision model, so the choice of Gemini for parsing was forced rather than optional. Within Gemini itself I rejected using the older `generativelanguage` REST SDK in favour of the unified `@google/genai` npm package, because it is Google's current, maintained SDK and the same client can later be pointed at Vertex AI without rework. Because the free tier does not expose an image `detail` knob, I deliberately did not try to emulate one by downscaling images pre-flight — the 10 MB cap and free-tier request budget already bound the image payload, and adding a resizing pipeline (sharp, extra buffers) for a setting I cannot pass to the API would have been pure overhead.
...
**How I implemented it.** `POST /api/jobs/<id>/summarize` checks the cache before it touches DeepSeek:
...
**Symptom.** Parse jobs that had clearly reached the model provider started failing in the validation step with `JSON.parse` errors complaining about unexpected tokens like backtick-and-"json".
...
**Cause.** Gemini, left to its own devices, happily formats its response as a display-ready Markdown block:
...
**Cause.** `maxOutputTokens: 1500` can be exhausted on a long, dense document, and Gemini reports this by returning `finishReason: "MAX_TOKENS"` rather than `"STOP"`. The pipeline only checked *whether* content came back, not *why* it stopped, so truncated JSON sailed into the validator.
